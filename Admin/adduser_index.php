<?php 
  ...
  if (isset($_POST['btn-submit'])) {
    require_once 'actions/signup.php';
  ...
  <div class="flex-container">
     <div class="flex-items">
       <div class="table-title">
        ...
        <div class="form-style-2-heading">ADD USERS</div>
        <form action="" method="post" name="form" id="formUser">
        <label for="field1"><span>First Name<span class="required">*</span></span><input type="text" class="input-field" name="firstname" id="firstname" value="" required></label>
        <label for="firstname"><span>Last Name<span class="required">*</span></span><input type="text" class="input-field" name="lastname" value="" required></label>
        <label for="field1"><span>Email<span class="required">*</span></span><input type="text" class="input-field" name="email" id="email" value="" required></label>
        <label for="field1"><span>Password<span class="required">*</span></span><input type="password" class="input-field" name="password" id="password" value="" required></label>    
        <label for="field4"><span>Role</span><select name="role" id="role" class="select-field" required>
        <option value="" hidden>Select Role</option>  
        <option value="1">Admin</option>
        <option value="2">Cashier</option>
        <option value="3">Stock Manager</option>
        </select></label>
        <label for="field4"><span>Supplier</span><select name="supplierid" class="select-field" name="supplierid" required></label>
        <option selected disabled value="">Select Branch</option>
        <?php
          
          $path = "actions/data.php";
          $branches = loadBranches();
          foreach ($branches as $branch) {
          echo "<option id='".$branch['id']."' value='".$branch['id']."'>".$branch['name']."</option>";
            }
        ?>
        </select>
        <div align="left">
          <button type="submit" class="btn btn-primary" value="Add" name="btn-submit"  style="font-size: 16px; font-weight: 700;">Add</button>
